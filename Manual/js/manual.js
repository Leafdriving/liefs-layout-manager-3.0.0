...
var CodeBlock = /** @class */ (function () {
    ...
    CodeBlock.byLabel = function (label) {
        for (var key in CodeBlock.instances)
            if (CodeBlock.instances[key].label == label)
                return CodeBlock.instances[key];
        return undefined;
    };
    CodeBlock.download = function (filename, text) {
    ...
    CodeBlock.downloadfile = function (el) {
        el = el.parentElement;
        var label = el.id.slice(0, -3);
        var codeblock = CodeBlock.byLabel(label);
    ...
    CodeBlock.prototype.build = function () {
        ...
        var H_I_I_I = h(this.label + "_buttons", "25px", 5, I(this.label + "_b2", "Show Html Only <button onclick=\"CodeBlock.downloadfile(this)\">Download</button>", centerButton, Pages.button(this.label + "_inner", 1)), I(this.label + "_b3", "Show Javascript Only", centerButton, Pages.button(this.label + "_inner", 2)), I(this.label + "_b4", "Show Rendered Only", centerButton, Pages.button(this.label + "_inner", 3)));
        var P_4Items = P(this.label + "_inner", all3, this.htmlDisplayCell, this.javascriptDisplayCell, this.evalDisplayCell);
        // let title = I(`${this.label}_title`,this.label +" - " + this.discription, h1, "30px")
        var launch = I(this.label + "_launch", "Launch in new Tab - (then view-source!)", "25px", centerButton, events({ onclick: function () {
                Object.assign(document.createElement('a'), {
                    target: '_blank',
                    href: "./Examples/" + THIS.label + ".html"
                }).click();
            } }));
        var displaycell1 = v(this.label + "_v0", 2, 
        // title,
        I(this.label + "_b1", "Show all 3 Inline", centerButton, Pages.button(this.label + "_inner", 0), "25px"), H_I_I_I, P_4Items, launch);
        var displaycell2 = v(this.label + "_v0_2", 2, 
        // title,
...
var leftText = css("leftText", "font-size: 25px;background-color: #ADD8E6;color:black;font-weight: bold;");
